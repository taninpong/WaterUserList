...
export class ProgressReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // },sessionStorage.getItem("selected_reg"));
    // if(this.selected_reg){
    //   this.regSelected2()
    // }
  ...
  calculateTable() {
    ...
    for (let dis of dises) {
      ...
      let total = 0;
      let total_build = 0;
      let complete_build = 0;
      let complete_ea = 0;
      let total_ea = 0;
      let unreach_building = 0;
      ...
      for (let ea of ea_temp) {
        ...
        let building_complete = 0;
        let building_unreach = 0;
        if(hassn1){
          let building = this.sn_1List.filter(sn1 => sn1.SN1_ID == dis.id + ea.EA).map(sn1 => sn1.SN1P2).reduce((a, b) => a.concat(b));
          building_complete = building.length;
          let unservey = building.filter(p2 => (p2.H1_3 != null && p2.H1_3 != 1) || (p2.H1_3 == null && p2.H1_2 == 4) || (p2.H1_3 == null && p2.H1_2 == 4))
          building_unreach = unservey.length;
          let household = building.map(p2 => p2.H3).filter(h3 => h3 != null).reduce((a, b) => a.concat(b)).filter(h3 => h3.status == 4);
          ea_complete = household.length;
          ea_completeByea = this.getSN1Status(ea).filter(st => st != 1).length == 0 ? 1 : 0;
        }
        let ea_total = Number(ea.Household);
        let building_total = Number(ea.Building);
        let ea_totalByea = 1;
        complete += ea_complete;
        total += ea_total;
        complete_build += building_complete;
        unreach_building += building_unreach;
        total_build += building_total;
      ...
      dis_map[dis.id] = {
        ...
        unreach_building: unreach_building,
    ...
    for (let tam of tams) {
      ...
      for (let dis of tam.children) {
        ...
        complete_build += dis_map[dis.id].complete_build;
        unreach_building += dis_map[dis.id].unreach_building;
        total_build += dis_map[dis.id].total_build;
    ...
    for (let amp of amps) {
      ...
      for (let tam of amp.children) {
        ...
        complete_build += tam_map[tam.id].complete_build;
        total_build += tam_map[tam.id].total_build;
        unreach_building += tam_map[tam.id].unreach_building;
    ...
    for (let cwt of cwts) {
      ...
      for (let amp of cwt.children) {
        ...
        unreach_building += amp_map[amp.id].unreach_building;
        complete_build += amp_map[amp.id].complete_build;
        total_build += amp_map[amp.id].total_build;
    ...
    for (let reg of regs) {
      ...
      for (let cwt of reg.children) {
        ...
        unreach_building += cwt_map[cwt.id].unreach_building;
        complete_build += cwt_map[cwt.id].complete_build;
        total_build += cwt_map[cwt.id].total_build;
  ...
  calculateSummary(data) {
    ...
    for (let area of Object.values(data)) {
      ...
      total += area['total'];
      complete_build += area['complete_build'];
      total_build += area['total_build'];
  ...
  back() {
    ...
    for (let i of this.selectedID) {
      summary_temp = {
        ...
        complete_build: children_temp[i]['complete_build'],
        total_build: children_temp[i]['total_build'],
  ...
  toJSON() {
    ...
    for (let area of this.displayData) {
      json.push({
        'รหัสภาค': area.id,
        'ชื่อ': area.name,
        'งานที่สำรวจแล้ว(รายครัวเรือน)': area.complete,
        'งานทั้งหมด(รายครัวเรือน)': area.total,
        'งานที่สำรวจแล้ว(รายอาคาร)': area.complete_build,
        'งานทั้งหมด(รายอาคาร)': area.total_build,
        
        'งานที่สำรวจแล้ว(รายเขตแจงนับ)': area.complete_ea,
    ...
    popupWin.document.write(
      `<html>
        ...
        <body onload="window.print()">
          <div class="container">
            <h3 class="text-center">รายงานความก้าวหน้า</h3>
