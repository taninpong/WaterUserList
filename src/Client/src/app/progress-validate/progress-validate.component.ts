...
@Component({
  selector: 'app-progress-validate',
  templateUrl: './progress-validate.component.html',
  styleUrls: ['./progress-validate.component.css']
})
export class ProgressValidateComponent implements OnInit {
    ...
    for (let dis of dises) {
      ...
      for (let ea of ea_temp) {
        ...
        if(hassn1){
          ...
          let household = building.map(p2 => p2.H3).filter(h3 => h3 != null).reduce((a, b) => a.concat(b)).filter(h3 => h3.status_approve != 0);
  ...
  toJSON() {
    ...
    for (let area of this.displayData) {
      json.push({
        'รหัสภาค': area.id,
        'ชื่อ': area.name,
        'งานที่ตรวจแล้ว(รายครัวเรือน)': area.complete,
        'งานทั้งหมด(รายครัวเรือน)': area.total,
        'งานที่ตรวจแล้ว(รายเขตแจงนับ)': area.complete_ea,
    ...
    popupWin.document.write(
      `<html>
        ...
        <body onload="window.print()">
          <div class="container">
            <h3 class="text-center">รายงานความก้าวหน้าการตรวจงาน</h3>
